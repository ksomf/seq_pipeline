...
rule pepr2tsv:
	...
	params:
		...
		pepr_cuttoff = 1e-15,
	run:
		broadpeak_colnames = [ 'chrom', 'start', 'end', 'name', 'score', 'strand', 'enrichment', 'stat', 'q_value' ]
		res = []
		for pepr_filename, pepr_condition in zip(input.pepr_peaks, params.pepr_conditions):
			df                = pd.read_csv(pepr_filename, sep='\t', names=broadpeak_colnames)
			df['strand']      = np.where( df['strand'] == '.', '*', df['strand'] )
			df['name']        = [ '_'.join(['pepr', pepr_condition, str(i)]) for i,name in enumerate(df['name']) ]
			df['method']      = 'pepr'
