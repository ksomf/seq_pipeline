...
rule download_assembly:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}.fasta"),
	params: url=lambda w: assembly2params[w.assembly]['fasta']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule download_gff:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}.gff"),
	params: url=lambda w: assembly2params[w.assembly]['gff']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule download_blacklist:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_blacklist.unsorted.bed"),
	params: url=lambda w: assembly2params[w.assembly]['encode_blacklist']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule download_tss:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_tss.bed"),
	params: url=lambda w: assembly2params[w.assembly]['tss']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule download_promoters:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_promoters.bed"),
	params: url=lambda w: assembly2params[w.assembly]['promoters']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule download_enhancers:
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_enhancers.bed"),
	params: url=lambda w: assembly2params[w.assembly]['enhancers']
	conda: "../envs/assemblies.yml"
	shell: "curl --location {params.url} | zcat > {output}"

rule generate_gtf:
	input:  os.path.join(config.get('reference_dir','reference'),"{assembly}.gff"),
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}.gtf"),
	conda: "../envs/assemblies.yml"
	shell:
		"""
...
rule generate_chromosome_sizes:
	input: os.path.join(config.get('reference_dir','reference'),"{assembly}.fasta"),
	output:
		sizes=os.path.join(config.get('reference_dir','reference'),"{assembly}_chromosome_sizes.tsv"),
		fai=os.path.join(config.get('reference_dir','reference'),"{assembly}.fasta.fai"),
...
rule generate_sorted_blacklist:
	input:
		blacklist=os.path.join(config.get('reference_dir','reference'),"{assembly}_blacklist.unsorted.bed"),
		sizes    =os.path.join(config.get('reference_dir','reference'),"{assembly}_chromosome_sizes.tsv"),
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_blacklist.bed"),
	conda: "../envs/assemblies.yml"
	shell: "bedtools sort -faidx {input.sizes} -i {input.blacklist} > {output}"

rule generate_whitelist:
	input:
		blacklist=os.path.join(config.get('reference_dir','reference'),"{assembly}_blacklist.bed"),
		sizes    =os.path.join(config.get('reference_dir','reference'),"{assembly}_chromosome_sizes.tsv"),
	output: os.path.join(config.get('reference_dir','reference'),"{assembly}_whitelist.bed"),
	conda: "../envs/assemblies.yml"
