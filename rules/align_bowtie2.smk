...
rule index_bowtie2:
	...
	output: expand(os.path.join(config['reference_dir'],'{assembly}.{ext}'), ext=bowtie2_index_extensions, allow_missing=True)
...
rule align_bowtie2:
	input:
		...
		index=expand(os.path.join(config['reference_dir'],'{assembly}.{ext}'), ext=bowtie2_index_extensions, allow_missing=True)
	output: 
