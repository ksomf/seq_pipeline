...
rule deq2tsv:
	...
	run:
		...
		for deq_filename, count_filename, deq_condition in zip(input.deq_peaks, input.deq_counts, params.deq_conditions):
			...
			df2               = pd.read_csv( count_filename, sep='\t', index=0 )
			df['chrom']       = df['seqnames']
			df['name']        = [ '_'.join(['deq', deq_condition, gn, s, e]) for gn, s, e in zip(df['main.gene'],df['start'],df['end'] ]
			df['method']      = 'deq'
			...
			df['min_padj']    = np.nanmin(df[['deseq2.padj', 'edeger.padj', 'qnb.padj']], axis=1) 
			...
			df['count_sig']   = np.sum(df2 > params.deq_peak_min_count, axis=1) == len(df2.colnames)
