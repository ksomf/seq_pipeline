...
rule star_generate_index:
	input:
		fasta=os.path.join(config.get('reference_dir','reference'),"{assembly}.fasta"),
		gtf  =os.path.join(config.get('reference_dir','reference'),"{assembly}.gtf"),
	output:
		index=expand(os.path.join(config.get('reference_dir','reference'),"{assembly}_star_index/{f}"), f=star_index_files, allow_missing=True),
	params:
...
rule star_align_pair_end:
	input:
		index=expand(os.path.join(config.get('reference_dir','reference'),"{assembly}_star_index/{f}"), assembly=config['assembly'], f=star_index_files, allow_missing=True),
		fastq=lambda w: sample_id2reads[w.sample_id]
		#fastq=expand("align/{sample_id}_{read}.fastq.gz", read=['R1', 'R2'], allow_missing=True),
	output:
		aligned_file="align/{sample_id}.star_aligned.unsorted.Aligned.out.bam",
	params:
...
rule star_sorted_bam:
	input:
		aligned_file="align/{sample_id}.star_aligned.unsorted.Aligned.out.bam",
	output:
		sorted_file=temp(local("align/{sample_id}.star_aligned.bam")),
	params:
